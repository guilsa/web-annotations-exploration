<div align="center">

# 🖍️ Web Annotations

**Highlight the web, comment in context, and share without an account or central database.**

</div>

Three approaches to sharing page comments, each designed as a standalone
browser extension. Plain JavaScript, with no build step or dependencies.

https://github.com/user-attachments/assets/822fd001-e806-40ae-9e4d-2a3f1f91a77e

## Variants

- 🔗 **Linkshare** · [`version_a`](version_a) — the link is the database: offline, portable, server-free.
- 🤝 **Pairshare** · [`version_b`](version_b) — live browser-to-browser sync over WebRTC.
- 📌 **Pinshare** · `version_c` — standalone comment sheets via local IPFS or an HTML file. *(Planned.)*

## Try it

Load `version_a/manifest.json` or `version_b/manifest.json` as a temporary
Firefox add-on, or load either directory unpacked in Chromium.

## Prior work

Inspired by [Textmarker](https://github.com/ufb/Textmarker).
